...
def process_batch_parallel(tokenizer: PreTrainedTokenizerFast, texts: List[str], max_length: int) -> Dict[str, np.ndarray]:
    ...
    def _tokenize_text(text: str) -> List[str]:
        # Split the text into *words*. This is the key to whole-word masking.
        # Use .split() for basic whitespace splitting, or a more robust
        # tokenizer like nltk.word_tokenize for better handling of punctuation, etc.
        return text.split()

    with ThreadPoolExecutor(max_workers=4) as executor:
        all_words = list(executor.map(_tokenize_text, texts))  # List of lists of *words*

    # Now, do the padding/truncation *after* the parallel processing,
    # and importantly, use is_split_into_words=True.
    encoding = tokenizer(
        all_words,
        ...
        return_offsets_mapping=False  # Not needed for word_ids now
    )

    # Use .word_ids() *directly*, and replace None with -1.
...
class CSVDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        item = {
            ...
            'index': idx, # ADDED: include index in the batch
        }
