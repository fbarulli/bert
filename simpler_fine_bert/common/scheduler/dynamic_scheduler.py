...
def create_scheduler(config: Dict[str, Any]) -> Optional[_LRScheduler]:
    """
    ...
    """
    try:
        if not bool(config.get('use_scheduler', True)):
            return None
            
        scheduler_type = config.get('scheduler_type', 'cosine')
        num_warmup_steps = int(config.get('warmup_steps', 0))
        num_training_steps = int(config.get('num_training_steps', 1000))
        min_lr_ratio = float(config.get('min_lr_ratio', 0.0))
        
        if scheduler_type == 'cosine':
            return lambda optimizer: WarmupCosineScheduler(
                optimizer,
            ...
            return lambda optimizer: WarmupLinearScheduler(
                optimizer,
                num_warmup_steps=num_warmup_steps,
                num_training_steps=num_training_steps,
                min_lr_ratio=min_lr_ratio
...
def get_scheduler_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """
    Extract scheduler configuration from main config.
    ...
    try:
        ...
        logger.error(f"Error extracting scheduler config: {str(e)}")
        raise